...
public class Parser {
    ...
    public ResultValue forStmt(boolean bExec, Debugger debug) throws Exception{
        ...
        if(forStmtScan.contains("in")) {
            ...
            forInStmt(bExec,debug,forStmtSplit[1],forStmtSplit[3]);
        }
    ...
    public ResultValue forInStmt(boolean bExec, Debugger debug, String cv_name, String arr_name)throws Exception{
        ResultValue cv = new ResultValue();
        ResultValue res = new ResultValue();
        //try to access the variable, if it is not declare then explicitly declare it to be of array data type
        try{
            cv = storageMgr.getVariable(this,cv_name);
        }
        //if fail then we declare
        catch (Exception e){

            //TODO: getArray(arr_name)
            // send in the String for the array name to check exist if not then
            // the function will raise error by itself

            //TODO: get the type of the array send in so it can declare
            // delete dummy type
            declareVariable("Int");
        }

        //TODO: THIS IS JUST A DUMMY RETURN;
        return res;
    ...
    public ResultValue forToStmt(boolean bExec, Debugger debug, boolean by)throws Exception{
        ...
        String cv_token_str = scan.getNext();            //getting the counting variable
        ...
        // At this step we are done with getting all the pieces of information for the actual execution
